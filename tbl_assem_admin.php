...
<body>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                     	...
                     	<a href="save_assem.php" class="btn btn-success" >Convert to Excel</a></tr>
						<a href="hapus_assem.php?id="onclick="return confirm('Anda yakin akan menghapus semua data evaluasi?')" class="btn btn-danger">Hapus</a><br><br>
	</div>
	</div>
	<div class="col-lg-12">
	...
	<?php
	include('conn.php');
	$query=mysql_query("SELECT * FROM tbl_evaluasi_assem ORDER BY ID_evaluasi_assem ASC") or die(mysql_error());
	if(mysql_num_rows($query)==0){
			...
			while($data=mysql_fetch_assoc($query)){
				echo'<tr>';
					echo '<td>'.$no.'</td>';
					echo '<td>'.$data['nama_penilai_assem'].'</td>';
					echo '<td>'.$data['nama_evaluasi_assem'].'</td>';
					echo '<td>'.$data['soal1_assem'].'</td>';
					echo '<td>'.$data['komentar1_assem'].'</td>';
					echo '<td>'.$data['soal2_assem'].'</td>';
					echo '<td>'.$data['komentar2_assem'].'</td>';
					echo '<td>'.$data['soal3_assem'].'</td>';
					echo '<td>'.$data['komentar3_assem'].'</td>';
					echo '<td>'.$data['date_evaluasi_assem'].'</td>';
					?><td align="center">
                		<a href="hapus_tbl_assem.php?id=<?php echo $data['ID_evaluasi_assem']; ?>"onclick="return confirm('Anda yakin akan menghapus data evaluasi?')" class="btn btn-xs btn-danger">Hapus</a>
          			</td><?php
				echo '</tr>';
				$no++;
			}
